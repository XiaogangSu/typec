...
class Main():
    ...
    def gps_pos_sta(self):
        ...
        for var in data_list:
            ...
            # print(temp)
            [x,y,z] = self.geodetic_to_geocentric(self.wgs84, temp[1], temp[2], temp[4])
            temp = temp+[x,y,z]
    ...
    def imgpos_sta(self):
        ...
        gps_del = gps_2-gps_1
        max_index = np.argmax(gps_del) #丢帧最大位置
        self.max_imgloss = int(gps_del[max_index]*20)-1
        # dellist = gps_del.tolist()
        # for i in range(len(dellist)):
        #     if dellist[i]<0:
        #         print('*******',i,' ',dellist[i])
